...
RUN cp "$(swift build --package-path /build -c release --show-bin-path)/ToDoApp" ./
...
COPY --chown=hummingbird:hummingbird Resources/volatile-config.json /app/config.json
